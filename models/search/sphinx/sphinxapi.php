<?php
	...
	class SphinxClient {
		...
		public function SetFilter($attribute, $values, $exclude = false) {
			...
			assert(umiCount($values));

			if (is_array($values) && umiCount($values)) {
		...
		public function SetFilterRange($attribute, $min, $max, $exclude = false) {
			...
			$this->_filters[] =
				['type' => SPH_FILTER_RANGE, 'attr' => $attribute, 'exclude' => $exclude, 'min' => $min, 'max' => $max];
		}
		...
		public function SetFilterFloatRange($attribute, $min, $max, $exclude = false) {
			...
			$this->_filters[] =
				['type' => SPH_FILTER_FLOATRANGE, 'attr' => $attribute, 'exclude' => $exclude, 'min' => $min, 'max' => $max];
		}
		...
		public function SetOverride($attrname, $attrtype, $values) {
			...
			assert(in_array($attrtype, [SPH_ATTR_INTEGER, SPH_ATTR_TIMESTAMP, SPH_ATTR_BOOL, SPH_ATTR_FLOAT, SPH_ATTR_BIGINT]));
			assert(is_array($values));
		...
		public function AddQuery($query, $index = '*', $comment = '') {
			...
			$req .= pack('N', umiCount($this->_weights)); // weights
			...
			// filters
			$req .= pack('N', umiCount($this->_filters));
			foreach ($this->_filters as $filter) {
				$req .= pack('N', strlen($filter['attr'])) . $filter['attr'];
				$req .= pack('N', $filter['type']);
				switch ($filter['type']) {
					case SPH_FILTER_VALUES:
						$req .= pack('N', umiCount($filter['values']));
			...
			// per-index weights
			$req .= pack('N', umiCount($this->_indexweights));
			...
			// per-field weights
			$req .= pack('N', umiCount($this->_fieldweights));
			...
			// attribute overrides
			$req .= pack('N', umiCount($this->_overrides));
			foreach ($this->_overrides as $key => $entry) {
				$req .= pack('N', strlen($entry['attr'])) . $entry['attr'];
				$req .= pack('NN', $entry['type'], umiCount($entry['values']));
		...
		protected function _ParseSearchResponse($response, $nreqs) {
			...
			for ($ires = 0; $ires < $nreqs && $p < $max; $ires++) {
				...
				// extract status
				list($umi_temp, $status) = unpack('N*', substr($response, $p, 4));
				$p += 4;
				$result['status'] = $status;
				if ($status != SEARCHD_OK) {
					list($umi_temp, $len) = unpack('N*', substr($response, $p, 4));
					$p += 4;
				...
				list($umi_temp, $nfields) = unpack('N*', substr($response, $p, 4));
				$p += 4;
				...
				list($umi_temp, $nattrs) = unpack('N*', substr($response, $p, 4));
				$p += 4;
				while ($nattrs-- > 0 && $p < $max) {
					...
					$p += $len;
					list($umi_temp, $type) = unpack('N*', substr($response, $p, 4));
					$p += 4;
					$attrs[$attr] = $type;
				}
				$result['attrs'] = $attrs;
				...
				// read match count
				list($umi_temp, $count) = unpack('N*', substr($response, $p, 4));
				$p += 4;
				list($umi_temp, $id64) = unpack('N*', substr($response, $p, 4));
				$p += 4;
				...
				while ($count-- > 0 && $p < $max) {
					...
					if ($id64) {
						...
						$p += 8;
						list($umi_temp, $weight) = unpack('N*', substr($response, $p, 4));
						$p += 4;
					} else {
					...
					foreach ($attrs as $attr => $type) {
						...
						if ($type == SPH_ATTR_FLOAT) {
							list($umi_temp, $uval) = unpack('N*', substr($response, $p, 4));
							$p += 4;
						...
						list($umi_temp, $val) = unpack('N*', substr($response, $p, 4));
						$p += 4;
		...
		public function UpdateAttributes($index, $attrs, $values, $mva = false) {
			...
			foreach ($values as $id => $entry) {
				...
				assert(umiCount($entry) == umiCount($attrs));
				foreach ($entry as $v) {
					if ($mva) {
						assert(is_array($v));
						foreach ($v as $vv) {
			...
			$req .= pack('N', umiCount($attrs));
			foreach ($attrs as $attr) {
				$req .= pack('N', strlen($attr)) . $attr;
				$req .= pack('N', $mva ? 1 : 0);
			}

			$req .= pack('N', umiCount($values));
			foreach ($values as $id => $entry) {
				$req .= sphPackU64($id);
				foreach ($entry as $v) {
					$req .= pack('N', $mva ? umiCount($v) : $v);
					if ($mva) {
						foreach ($v as $vv) {
							$req .= pack('N', $vv);
						}
