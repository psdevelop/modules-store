<?php
	...
	class invoicePayment extends payment {
		...
		public function sendInvoiceMail($template = 'default') {
			...
			$legalPerson = $this->order->getValue('legal_person');
			$variables = [
			...
			if ($module->isUsingUmiNotifications()) {
				...
				try {
					...
					$subject = emarket::parseTemplateForMail($subjectTemplate, $variables, false, $legalPerson);
					$content = emarket::parseTemplateForMail($contentTemplate, $variables, false, $legalPerson);
				} catch (Exception $e) {
					// nothing
				}
			}
			...
			$person = umiObjectsCollection::getInstance()->getObject($legalPerson);
