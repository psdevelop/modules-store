...
class PlanfixHelper
{
    ...
    public static function getClientError($key)
    {
        return static::getErrors()[(string)$key] ?? 'Неизвестная ошибка!';
    }

    public static function parseError($errorData)
    {
        $errorCode = $errorData['error_code'] ?? null;
        return (object)[
            'description' => static::getClientError($errorCode),
            'code' => $errorCode,
            'errorString' => $errorData['error_str'] ?? ''
        ];
    }
}