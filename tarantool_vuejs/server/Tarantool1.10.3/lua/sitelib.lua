...
function sitelib.httptypejson(code,data)
    return {
        status = code,
        headers = {
...
function sitelib.httptypehtml(code,data)
 return {
     status = code,
     headers = {
...
function sitelib.executecode(s,p,env)
...
local function LoadCode(code, environ)
    local f=assert(loadstring(code))
    setfenv(f,environ)
    return f
end

function sitelib.glue(s,p)
    local file = sitelib.readfile(p)
    local m = '{{content}}'
    for v in s:gmatch(m) do
        s=s:gsub(m,tostring(file==nil and "" or file),1)
    end
    return s
end

function sitelib.readfile(path)
 local open = io.open
...
function sitelib.fiomin(fio)
    local ret = {}
    local i = 0
    for token in string.gmatch(fio, "[^%s]+") do
        i = i + 1
        ret[i] = token
    end
    return ret[1] .. ' ' .. ret[2]:sub(1,2) .. '. ' .. ret[3]:sub(1,2) .. '.'
end

-- Проверяет на наличие значений
function sitelib.enabled(val)
 if val == nil then return true end
 return val == true or (val == "1" or val == "true" or val == "on")
end

-- Удаление всех пробелов до и после строки
function sitelib.trim(s)
 return gsub(gsub(s, "^%s+", ""), "%s+$", "")
end

function sitelib.selecttestuser()
